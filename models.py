...
class Author(db.Model):   # class Author will be use to create table in db
    ...
    @staticmethod
    def get_filter_argument(columnt_name: InstrumentedAttribute, value: str, operator: str) -> BinaryExpression:
        operator_mapping = {
            '==': columnt_name == value,
            'gte': columnt_name >= value,
            'gt': columnt_name > value,
            'lte': columnt_name <= value,
            'lt': columnt_name < value
        }
